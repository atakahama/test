...
class RayBatchStream(IterableDataset):
    ...
    def __iter__(self):
        ...
        while True:
            ...
            """
            #print("BBB", collated_batch.keys()) #この時点でマスクが含まれていることを確認
            ...
            # 追加するコード - マスクの処理
            if "mask" in collated_batch:
                # collated_batch にマスクが含まれていても batch に含まれていない場合
                if "mask" not in batch:
                    # マスクのタイプを確認
                    if isinstance(collated_batch["mask"], list):
                        # こちらで処理される
                        # リスト型の場合は別の処理が必要
                        # 例：各画像に対応するマスクから正しい値を抽出
                        c = ray_indices[:, 0].long()  # 画像インデックス
                        y = ray_indices[:, 1].long()  # y座標
                        x = ray_indices[:, 2].long()  # x座標
                        ...
                        for i in range(len(ray_indices)):
                            ...
                            x_pos = x[i].item()
                            # 画像インデックスが有効範囲内かチェック
                            if img_idx < len(collated_batch["mask"]):
                                mask_value = collated_batch["mask"][img_idx][y_pos, x_pos]
                                sampled_masks.append(mask_value)
                            else:
                                # デフォルト値（例：すべて1）
                                sampled_masks.append(torch.ones(1, device=self.device))
                        
                        # マスク値をテンソルとしてスタック
                        batch["mask"] = torch.stack(sampled_masks).to(self.device)
                    else:

                        # テンソル型の場合は元の実装通り
                        c, y, x = (i.flatten() for i in torch.split(ray_indices, 1, dim=-1))
                        batch["mask"] = collated_batch["mask"][c, y, x].to(self.device)
                
                # batch にマスクが含まれている場合は、デバイスが正しいことを確認
                elif batch["mask"].device != self.device:
